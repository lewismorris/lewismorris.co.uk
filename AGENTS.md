...
**Note:** `groq` is listed in dependencies but not currently used in the codebase (likely remnant from a previous CMS integration).

## Project Structure

```
lewismorris.co.uk/
├── astro.config.mjs       # Astro config: site URL, sitemap, Tailwind via Vite
...
│   │   ├── BlogLayout.astro # Blog post layout: title, date, Prose wrapper
...
│       └── writing/        # Markdown blog posts (file-based routing)
│           ├── *.md        # Posts use frontmatter + BlogLayout
```

## Routing & Content

- **File-based routing:** `src/pages/` maps directly to URLs.
- **Blog posts:** Markdown files in `src/pages/writing/*.md` use `layout: ../../layouts/BlogLayout.astro` in frontmatter.
- **Post frontmatter:** `title`, `date`, `description` (optional).
- **Post listing:** Homepage uses `Astro.glob('./writing/*.md')` and sorts by date descending.
...
2. **BlogLayout.astro** — Wraps `Layout`, adds article header (title, formatted date), `Prose` for content.
...
2. **New blog posts:** Add `.md` in `src/pages/writing/` with frontmatter `layout`, `title`, `date`, and optional `description`.
